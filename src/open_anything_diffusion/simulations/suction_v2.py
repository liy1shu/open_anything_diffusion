...
class PMSuctionSim:
    def __init__(self, obj_id: str, dataset_path: str, gui: bool = False):
        ...
        self.gui = gui
        self.gripper = FloatingSuctionGripperV2(self.render_env.client_id)
    ...
    def reset_gripper(self, target_link):
        # BUG????
        # Restore the current link angle, and reset to the angle after resetting gripper (to avoid bug)
    ...
    def set_gripper_pose(self, pos, ori):
        self.gripper.set_state(pos, ori)
    ...
    def teleport(
        ...
        curr_pos = self.get_joint_value(target_link)  # To avoid bugs
        for id, (point, contact_vector) in enumerate(zip(points, contact_vectors)):
            ...
            o_teleport = R.from_matrix(R_teleport @ R_gripper).as_euler(
                "xyz", degrees=True
            )
            move_fn = self.gripper.get_move_fn(torch.from_numpy(point), o_teleport)
            max_steps = 500
            curr_steps = 0
            goal_reached = False
            # self.gripper.set_velocity(-contact_vector * 0.4, [0, 0, 0])
            while not goal_reached and curr_steps < max_steps:
                self.gripper.update_state()
                ...
                if video_writer is not None and curr_steps % 50 == 49:
                    ...
                    image = image[:, :, :3]
                    # if curr_steps == 0:
                    #     cv2.imwrite('/home/yishu/open_anything_diffusion/src/open_anything_diffusion/simulations/logs/simu_eval/video_assets/static_frames/reset_gripper.jpg', image)
                    # if curr_steps == max_steps - 1:
                    #     cv2.imwrite('/home/yishu/open_anything_diffusion/src/open_anything_diffusion/simulations/logs/simu_eval/video_assets/static_frames/attach_gripper.jpg', image)
                ...
                if curr_steps % 1 == 0:
                    contact = self.gripper.detect_contact(
                        self.render_env.obj_id,
                        self.render_env.link_name_to_index[target_link],
                    )
    ...
    def attach(self, target_link: str):
        curr_pos = self.get_joint_value(target_link)  # To avoid bugs
        self.gripper.activate(
            self.render_env.obj_id, self.render_env.link_name_to_index[target_link]
        )
        self.set_joint_state(target_link, curr_pos)
    ...
    def pull_with_constraint(self, direction, n_steps: int = 50, target_link: str = ""):
        # Link info
        link_index = self.render_env.link_name_to_index[target_link]
        info = p.getJointInfo(
        ...
        for _ in range(n_steps):
            pull_fn = self.gripper.get_pull_fn(direction.numpy())
            self.gripper.set_pull_cmds(pull_fn()[0])
            p.stepSimulation(self.render_env.client_id)
            if self.gui:
                time.sleep(1 / 240.0)
            self.gripper.update_state()
        ...
        if curr_pos < lower < upper or curr_pos > lower > upper:
            self.reset_gripper(target_link)
            p.resetJointState(
...
def run_trial(
    ...
    pc_obs = env.render(filter_nonobj_pts=True, n_pts=n_pts)
    success = env.detect_success(target_link=target_link)
    ...
    while global_step < n_steps:
        ...
        for traj_step in range(pred_trajectory.shape[1]):
            ...
            if last_step_grasp_point is None or lev_diff[0] > lev_diff_thres:
                ...
                # image.save('/home/yishu/open_anything_diffusion/src/open_anything_diffusion/simulations/logs/simu_eval/video_assets/static_frames/attach_gripper.jpg')
                best_flow = pred_flow[link_ixs][best_flow_ixs[best_flow_ix_id]]
                best_point = P_world[link_ixs][best_flow_ixs[best_flow_ix_id]]
...
def run_trial_with_history_filter(
    ...
    env.attach(target_link=target_link)
    use_history = False
    ...
    while not success and global_step < n_steps:
        ...
        if last_step_grasp_point is not None:  # Still grasping!
            gripper_tip_pos, _ = p.getBasePositionAndOrientation(env.gripper.mount_id)
