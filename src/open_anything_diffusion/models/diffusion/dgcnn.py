...
class MLP(ModelMixin, ConfigMixin):
    ...
    def __init__(self, in_channels, sample_size, time_embed_dim, emb_dims=512):
        ...
        out_channels = [256, 512, 1024, 2048, 512, 128]
        # out_channels = [256, 2048, 2048, 2048]

        # MLP components
        self.fc1 = nn.Linear(in_channels + time_embed_dim, out_channels[0], bias=True)
        self.fc2 = nn.Linear(
            out_channels[0] + time_embed_dim, out_channels[1], bias=True
        )
        self.fc3 = nn.Linear(
            out_channels[1] + time_embed_dim, out_channels[2], bias=True
        )
        self.fc4 = nn.Linear(
            out_channels[2] + time_embed_dim, out_channels[3], bias=True
        )
        self.fc5 = nn.Linear(
            out_channels[3] + time_embed_dim, out_channels[4], bias=True
        )
        self.fc6 = nn.Linear(
            out_channels[4] + time_embed_dim, out_channels[5], bias=True
        )
        self.fc7 = nn.Linear(out_channels[5] + time_embed_dim, emb_dims, bias=True)

    def forward(self, x, timestep, context, return_dict: bool = True):
        # time embedding
        timesteps = timestep
        ...
        x = torch.cat((x, emb), dim=1)  # (B, d+64, N)

        x = x.permute(0, 2, 1)  # B x N x d+64
        ...
        x = x.permute(0, 2, 1)  # B x d+64 x N
...
class DGCNN(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        # goal embedding
        batch_size, num_dims, traj_len, num_points = x.size()
        x = x.reshape(batch_size, num_dims * traj_len, num_points)
        goal_pcd = context.transpose(1, 2)
